 ...
 *
 * MSX2 uses the V9938 VDP which has a programmable 512-color palette.
 * SCREEN 5 (256x212, 16 colors) is the most common mode for games.
 * Palette stored in VDP registers: 0RRRGGGB BB000000 format.
 *
 * References:
...
(function () {
   ...
   */
  var MSX2ConsoleMode = function () {
    pskl.consoles.ConsoleMode.call(this, {
      id: 'msx2',
      name: 'MSX2 (V9938)',
      bodyClass: 'console-msx2',
  ...
  pskl.utils.inherit(MSX2ConsoleMode, pskl.consoles.ConsoleMode);

  /**
   * Snaps an 8-bit RGB value (0-255) to nearest V9938 3-bit value.
   * V9938 uses 3 bits per channel = 8 levels (0-7).
   * @param {number} value8bit - RGB component value 0-255
   * @return {number} Snapped 8-bit value (one of 8 valid V9938 values)
   */
  MSX2ConsoleMode.prototype.snapTo3Bit = function (value8bit) {
    var value3bit = Math.round(value8bit * 7 / 255);
    return Math.round(value3bit * 255 / 7);
  };

  /**
   * Gets the 3-bit V9938 value (0-7) from an 8-bit value.
   * @param {number} value8bit - RGB component value 0-255
   * @return {number} V9938 3-bit value 0-7
   */
  MSX2ConsoleMode.prototype.to3Bit = function (value8bit) {
    return Math.round(value8bit * 7 / 255);
  };

  /**
   * Snaps a hex color to the nearest valid V9938 RGB333 color.
   * @param {string} hexColor - Hex color string (e.g., '#FF8040')
   * @return {string} Snapped hex color string
   */
  MSX2ConsoleMode.prototype.snapColorToRGB333 = function (hexColor) {
  ...
  /**
   * Converts a hex color to V9938 palette register format (2 bytes).
   * Format: 0RRR0GGG 0BBB0000 (MSX2 VDP palette register format)
   * @param {string} hexColor - Hex color string
   * @return {Uint8Array} 2-byte array in V9938 palette format
   */
  MSX2ConsoleMode.prototype.colorToPaletteBytes = function (hexColor) {
    var tc = window.tinycolor(hexColor);
    var rgb = tc.toRgb();
    ...
    // V9938 palette format: 0RRR0GGG 0BBB0000
    // Byte 0: 0RRR0GGG (R in bits 4-6, G in bits 0-2)
    // Byte 1: 0BBB0000 (B in bits 4-6)
    var byte0 = (r3 << 4) | g3;
    var byte1 = (b3 << 4);
   ...
   */
  MSX2ConsoleMode.prototype.getPaletteData = function () {
   ...
   */
  MSX2ConsoleMode.prototype.isValidColor = function (color) {
    return true;
  };

  /**
   * Creates and returns a new MSX2 console mode instance.
   * @return {MSX2ConsoleMode}
   */
  ns.createMSX2Mode = function () {
    return new MSX2ConsoleMode();
  };
