/**
 * Controller for GBA 8BPP tile + palette export.
 *
 * 8BPP linear format: 64 bytes per 8x8 tile
 *   - Each byte contains 1 pixel (palette index 0-255)
 *   - Pixels stored row by row, left to right
 *   - Index 0 is typically transparent
 *
 * Palette format: 512 bytes for 256-color palette
 *   - Each color is 2 bytes in BGR555 little-endian format
 *   - byte0 = GGGRRRRR, byte1 = 0BBBBBGG
 *
 * Exports:
 *   - .8bpp file: Tile data (all frames concatenated)
...
(function () {
  ...
  /** @const {number} Max colors per sprite (255 + transparent). */
  var MAX_COLORS = 255;

  ns.Gba8bppExportController = function (piskelController) {
    this.piskelController = piskelController;
    this.colorMap = {};       // Maps color int -> index (0-255)
    this.paletteColors = [];  // Array of hex colors in order
  };

  pskl.utils.inherit(ns.Gba8bppExportController,
    pskl.controller.settings.AbstractSettingController);

  ns.Gba8bppExportController.prototype.init = function () {
    this.validateAndDisplay_();

    var tilesBtn = document.querySelector('.gba8bpp-download-tiles-button');
    var palBtn = document.querySelector('.gba8bpp-download-pal-button');
    var bothBtn = document.querySelector('.gba8bpp-download-both-button');
   ...
   * @private
   */
  ns.Gba8bppExportController.prototype.validateAndDisplay_ = function () {
    ...
    var warning = document.querySelector('.gba8bpp-color-warning');
    if (warning) {
      warning.style.display = hasTooManyColors ? 'flex' : 'none';
    }

    // Build color map for export (up to 255 colors + transparent)
    this.colorMap = {};
    this.paletteColors = [];
    this.colorMap[0] = 0;  // Transparent maps to index 0
    this.paletteColors.push(null);  // Index 0 = transparent

    // Map up to 255 non-transparent colors
    ...
    while (this.paletteColors.length < 256) {
      this.paletteColors.push('#000000');
    }

    // Update download info
    var tilesPerFrame = (width / 8) * (height / 8);
    var totalTileBytes = frameCount * tilesPerFrame * 64;  // 64 bytes per tile
    var paletteBytes = 512;  // 256 colors × 2 bytes

    var tilesInfo = document.querySelector('.gba8bpp-tiles-info');
    var palInfo = document.querySelector('.gba8bpp-pal-info');
   ...
   * @private
   */
  ns.Gba8bppExportController.prototype.onDownloadTilesClick_ = function () {
    var data = this.generate8bppData_();
    if (data) {
      var fileName = this.getPiskelName_() + '.8bpp';
   ...
   * @private
   */
  ns.Gba8bppExportController.prototype.onDownloadPaletteClick_ = function () {
   ...
   * @private
   */
  ns.Gba8bppExportController.prototype.onDownloadBothClick_ = function () {
  ...
  /**
   * Generates the binary 8BPP tile data for all frames.
   *
   * GBA 8BPP uses linear format:
   *   - Each byte = 1 pixel (palette index 0-255)
   *   - 64 bytes per 8x8 tile (8 rows × 8 bytes per row)
   *
   * @return {Uint8Array} The 8BPP file bytes
   * @private
   */
  ns.Gba8bppExportController.prototype.generate8bppData_ = function () {
    var width = this.piskelController.getWidth();
    var height = this.piskelController.getHeight();
    var frameCount = this.piskelController.getFrameCount();
    ...
    var totalBytes = frameCount * tilesPerFrame * 64;  // 64 bytes per tile
   ...
   * @return {Uint8Array} The palette file bytes (512 bytes)
   * @private
   */
  ns.Gba8bppExportController.prototype.generatePaletteData_ = function () {
    var gbaMode = pskl.app.consoleRegistry.get('gba8bpp');
  ...
  /**
   * Encodes a single 8x8 tile to 64 bytes in GBA 8BPP linear format.
   *
   * GBA 8BPP linear format:
   *   - 8 bytes per row (8 pixels, 1 byte per pixel)
   *   - Each byte is the palette index (0-255)
   ...
   * @return {Uint8Array} 64 bytes for this tile
   * @private
   */
  ns.Gba8bppExportController.prototype.encodeTile_ = function (
    pixels, imgWidth, startX, startY
  ) {
    var tile = new Uint8Array(64);
    var byteIdx = 0;
    ...
    for (var y = 0; y < 8; y++) {
      for (var x = 0; x < 8; x++) {
        ...
        var colorIdx = this.getColorIndex_(
          pixels[idx], pixels[idx + 1], pixels[idx + 2], pixels[idx + 3]
        );

        // One byte per pixel
        tile[byteIdx++] = colorIdx;
      }
    }
  ...
  /**
   * Maps an RGBA pixel to a color index 0-255.
   * @param {number} r - Red component (0-255)
   * @param {number} g - Green component (0-255)
   * @param {number} b - Blue component (0-255)
   * @param {number} a - Alpha component (0-255)
   * @return {number} Color index 0-255
   * @private
   */
  ns.Gba8bppExportController.prototype.getColorIndex_ = function (
    r, g, b, a
  ) {
    if (a < 128) {
      return 0;
    ...
    console.warn('Unmapped color during GBA 8BPP export:', r, g, b);
    return 1;
  };
   ...
   * @private
   */
  ns.Gba8bppExportController.prototype.getPiskelName_ = function () {
