/**
 * Controller for MSX2+ 4BPP tile + V9958 palette export.
 *
 ...
 *   - Each color is 2 bytes in V9958 format
 *   - Format: 0RRR0GGG 0BBB0000 (RGB333, same as V9938)
 *
 * Note: V9958 also supports YJK color mode for backgrounds, but sprites
 * use the same 4BPP format as MSX2.
 *
 * Exports:
 *   - .4bpp file: Tile data (all frames concatenated)
 *   - .pal file: Palette data (V9958 format, 32 bytes)
 *
 * References:
...
(function () {
  ...
  ns.Msx2plus4bppExportController = function (piskelController) {
    this.piskelController = piskelController;
    this.colorMap = {};
    this.paletteColors = [];
  };

  pskl.utils.inherit(ns.Msx2plus4bppExportController,
    pskl.controller.settings.AbstractSettingController);

  ns.Msx2plus4bppExportController.prototype.init = function () {
    this.validateAndDisplay_();

    var tilesBtn = document.querySelector('.msx2plus4bpp-download-tiles-button');
    var palBtn = document.querySelector('.msx2plus4bpp-download-pal-button');
    var bothBtn = document.querySelector('.msx2plus4bpp-download-both-button');
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.validateAndDisplay_ = function () {
    ...
    var warning = document.querySelector('.msx2plus4bpp-color-warning');
    if (warning) {
    ...
    var tilesInfo = document.querySelector('.msx2plus4bpp-tiles-info');
    var palInfo = document.querySelector('.msx2plus4bpp-pal-info');
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.onDownloadTilesClick_ = function () {
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.onDownloadPaletteClick_ = function () {
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.onDownloadBothClick_ = function () {
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.generate4bppData_ = function () {
    var width = this.piskelController.getWidth();
    var height = this.piskelController.getHeight();
    var frameCount = this.piskelController.getFrameCount();
  ...
  /**
   * Generates the binary palette data in V9958 format.
   * @return {Uint8Array} The palette file bytes (32 bytes)
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.generatePaletteData_ = function () {
    var msx2plusMode = pskl.app.consoleRegistry.get('msx2plus');
    ...
    for (var i = 0; i < 16; i++) {
      ...
      } else if (msx2plusMode) {
        var palBytes = msx2plusMode.colorToPaletteBytes(color);
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.encodeTile_ = function (
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.getColorIndex_ = function (
    r, g, b, a
  ) {
    if (a < 128) {
      return 0;
   ...
   * @private
   */
  ns.Msx2plus4bppExportController.prototype.getPiskelName_ = function () {
