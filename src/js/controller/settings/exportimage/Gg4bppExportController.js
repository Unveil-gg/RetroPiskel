/**
 * Controller for Sega Game Gear 4BPP tile + palette export.
 *
 * 4BPP format: 32 bytes per 8x8 tile (same as SMS/Genesis)
 ...
 *   - Each color is 2 bytes in GG CRAM format (little-endian)
 *   - Format: GGGGRRRR (low), ----BBBB (high) = RGB444
 *
 * Exports:
 *   - .4bpp file: Tile data (all frames concatenated)
 *   - .pal file: Palette data (GG CRAM format, 32 bytes)
 *
 * References:
...
(function () {
  ...
  ns.Gg4bppExportController = function (piskelController) {
    this.piskelController = piskelController;
    this.colorMap = {};       // Maps color int -> index (0-15)
    this.paletteColors = [];  // Array of hex colors in order
  };

  pskl.utils.inherit(ns.Gg4bppExportController,
    pskl.controller.settings.AbstractSettingController);

  ns.Gg4bppExportController.prototype.init = function () {
    this.validateAndDisplay_();

    var tilesBtn = document.querySelector('.gg4bpp-download-tiles-button');
    var palBtn = document.querySelector('.gg4bpp-download-pal-button');
    var bothBtn = document.querySelector('.gg4bpp-download-both-button');
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.validateAndDisplay_ = function () {
    ...
    var warning = document.querySelector('.gg4bpp-color-warning');
    if (warning) {
    ...
    var paletteBytes = 32;  // 16 colors × 2 bytes (GG CRAM)

    var tilesInfo = document.querySelector('.gg4bpp-tiles-info');
    var palInfo = document.querySelector('.gg4bpp-pal-info');
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.onDownloadTilesClick_ = function () {
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.onDownloadPaletteClick_ = function () {
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.onDownloadBothClick_ = function () {
   ...
   * GG 4BPP format: packed pixels, 4 bytes per row (same as SMS).
   * @return {Uint8Array} The 4BPP file bytes
   * @private
   */
  ns.Gg4bppExportController.prototype.generate4bppData_ = function () {
    var width = this.piskelController.getWidth();
    var height = this.piskelController.getHeight();
    var frameCount = this.piskelController.getFrameCount();
  ...
  /**
   * Generates the binary palette data in Game Gear CRAM format.
   * GG uses 2 bytes per color (RGB444, little-endian).
   * @return {Uint8Array} The palette file bytes (32 bytes)
   * @private
   */
  ns.Gg4bppExportController.prototype.generatePaletteData_ = function () {
    var ggMode = pskl.app.consoleRegistry.get('gamegear');
    var bytes = new Uint8Array(32);  // 16 colors × 2 bytes
    ...
    for (var i = 0; i < 16; i++) {
      ...
      if (i === 0 || color === null) {
        ...
        bytes[byteOffset] = 0x00;      // Low byte: GGGGRRRR
        bytes[byteOffset + 1] = 0x00;  // High byte: ----BBBB
      } else if (ggMode) {
        var cramBytes = ggMode.colorToCRAMBytes(color);
        bytes[byteOffset] = cramBytes[0];      // Low byte
        bytes[byteOffset + 1] = cramBytes[1];  // High byte
      } else {
        // Fallback: simple conversion to RGB444
        var tc = window.tinycolor(color).toRgb();
        var r4 = Math.round(tc.r * 15 / 255);
        var g4 = Math.round(tc.g * 15 / 255);
        var b4 = Math.round(tc.b * 15 / 255);
        bytes[byteOffset] = (g4 << 4) | r4;  // GGGGRRRR
        bytes[byteOffset + 1] = b4;          // ----BBBB
      }
    }
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.encodeTile_ = function (
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.getColorIndex_ = function (
    r, g, b, a
  ) {
    if (a < 128) {
      return 0;
    ...
    console.warn('Unmapped color during GG 4BPP export:', r, g, b);
    return 1;
  };
   ...
   * @private
   */
  ns.Gg4bppExportController.prototype.getPiskelName_ = function () {
